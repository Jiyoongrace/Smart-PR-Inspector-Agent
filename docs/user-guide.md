...
7. [PR 승인 및 머지하기](#7-pr-승인-및-머지하기)
8. [AI로 PR 자동 생성하기](#8-ai로-pr-자동-생성하기)
9. [Slack 연동 설정하기](#9-slack-연동-설정하기)
10. [도메인 문서 등록하기 (RAG)](#10-도메인-문서-등록하기-rag)
11. [컨벤션 룰 커스터마이징](#11-컨벤션-룰-커스터마이징)
...
```
PR 수집 → FORK [컨벤션 검사 + 테스트 생성 (병렬)] → JOIN
       → 아키텍처 룰 점검 (HITL 분기)
           ├── 위반 → 시니어 승인 대기 → 승인/반려
           └── 통과 → 테스트 실행
       → 테스트 실행 (재시도 분기)
       → 영향도 분석 → 도메인 설명 (Hybrid RAG) → 문서 동기화
       → GitHub 코멘트 → Slack 알림
```

> **병렬 실행**: `컨벤션 검사`와 `테스트 생성`은 Fork/Join으로 동시 실행됩니다.
> **HITL**: 아키텍처 룰 위반 시 시니어 리뷰어의 승인을 대기합니다. 반려 시 PR이 즉시 차단됩니다.
> **재시도**: 테스트가 실패하면 AI가 코드를 수정해 최대 3회 재시도합니다.
> **Hybrid RAG**: 도메인 설명은 Dense + Sparse + Re-ranking으로 정밀 검색합니다.
...
우측 채팅 패널에서 AI와 대화하며 분석 결과를 더 깊이 이해할 수 있습니다.
...
PR 분석이 완료되면 분석 결과 **개요 탭** 하단에 "PR 액션" 패널이 나타납니다.

### PR 승인하기

1. 분석 결과의 **개요 탭**으로 이동
2. 하단 "PR 액션" 패널에서 **"PR 승인"** 버튼 클릭
3. GitHub에 Approve Review가 자동으로 제출됨

> GitHub Token에 `pull_requests: write` 권한이 필요합니다.

### PR 머지하기

1. 머지 방식을 드롭다운에서 선택:
   - **Squash Merge** (기본) — 커밋을 1개로 합쳐서 깔끔하게 머지
   - **Merge Commit** — 모든 커밋 이력을 그대로 유지
   - **Rebase Merge** — 선형 이력 유지

2. **"PR 머지"** 버튼 클릭

> PR에 충돌이 있거나 필수 체크가 통과되지 않은 경우 오류 메시지가 표시됩니다.
...
커밋 내역을 AI가 분석해서 PR 제목과 본문을 자동으로 작성해줍니다.

### 사용 방법

1. 좌측 사이드바에서 **"생성" 탭** (GitMerge 아이콘) 클릭
2. 아래 정보를 입력:
   - **레포지토리**: `owner/repo` 형식 (예: `acme/backend`)
   - **Head 브랜치**: PR의 소스 브랜치 (예: `feature/login`)
   - **Base 브랜치**: 병합 대상 브랜치 (예: `main`)
   - **Draft 여부**: 체크 시 Draft PR로 생성
3. **"PR 생성"** 버튼 클릭
4. AI가 커밋을 분석해 PR 제목 + 변경 사항 + 체크리스트 자동 작성
5. 생성된 PR 번호와 GitHub 링크가 표시됨

> Claude API 크레딧이 없는 경우 커밋 메시지 기반으로 자동 생성됩니다.

---

## 9. Slack 연동 설정하기

Slack을 연동하면 분석 완료 후 Slack으로 알림이 오고, Slack에서도 승인/거부할 수 있습니다.
...
| 버튼 | 동작 |
|------|------|
| **✅ 승인** | GitHub Approve Review 제출 + "~~~ 기능 PR 승인했습니다" 메시지 표시 |
| **🔄 수정 요청** | 수정 영역 선택 메뉴 표시 (코드 로직/컨벤션/테스트/문서/보안/아키텍처) → 선택 후 "~~~ 기능 [영역] 수정 요청하였습니다" 메시지 + GitHub Request Changes 제출 |
| **📋 상세 보기** | GitHub PR 페이지로 이동 |

> **참고**: PR 승인/수정 요청은 **Slack에서만** 처리합니다. 웹 대시보드에서는 **머지만** 가능합니다.

---

## 10. 도메인 문서 등록하기 (Hybrid RAG)

팀의 내부 문서를 등록하면 AI가 **Hybrid RAG (Dense + Sparse + Re-ranking)**으로 정밀하게 검색하여 비즈니스 영향도를 설명합니다.

### 검색 원리

등록된 문서는 두 가지 인덱스에 동시 저장됩니다:
- **Dense (ChromaDB)**: 의미적 유사도로 검색 (예: "결제 프로세스" ↔ "PG사 연동")
- **Sparse (BM25)**: 키워드 정확 매칭으로 검색 (예: `validate_card` 함수명)

검색 시 두 결과를 RRF로 통합한 후, Cross-Encoder로 최종 재순위하여 Top-5 문서를 LLM에 전달합니다.
...
# 폴더 내 모든 .md 파일 Hybrid 인덱싱 (Dense + Sparse)
count = ingester.ingest_markdown_dir("./docs/domain/")
print(f"{count}개 청크 Hybrid 인덱싱 완료")
```
