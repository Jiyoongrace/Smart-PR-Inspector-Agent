...
```
시니어 리뷰 부담 절감 │ 컨벤션 위반 자동 탐지 │ Hybrid RAG 도메인 분석 │ HITL 아키텍처 통제
```
...
└──────────────────────────────┬───────────────────────────────────────┘
                               ▼
                ┌──────────────────────────┐
                │  [1] fetch               │  PR 데이터 수집 (PyGithub)
                └──────────────┬───────────┘
                               ▼
                ┌──────────────────────────┐
                │  [2] convention   🟧 LLM │  AST + RAG + LLM 컨벤션
                └──────────────┬───────────┘
                               ▼
                ┌──────────────────────────┐
                │  [3] test_gen     🟧 LLM │  BDD 시나리오 생성
                └──────────────┬───────────┘
                               ▼
                ┌──────────────────────────┐
                │  [4] arch_review  🟪 분기│  아키텍처 룰 점검
                └────┬─────────────────┬───┘
              위반 │                 │ 통과
                   ▼                 │
          ┌────────────────┐         │
        ...
        └────┬────────────┘         │
             └──────►────────┬──────┘
                ...
                └──────────────┬───────────┘
                               ▼
                ┌──────────────────────────┐
                │  [8] domain_explain 🟧🔍 │  Hybrid RAG 비즈니스 영향도
                └──────────────┬───────────┘
                               ▼
                ┌──────────────────────────┐
                │  [9] doc_sync     🔵     │  Swagger 변경 감지
                └──────────────┬───────────┘
                               ▼
                ┌──────────────────────────┐
                │  [10] comment     🟧     │  PR Health Score + 리포트
                └──────────────┬───────────┘
                               ▼
                ┌──────────────────────────┐
                │  [11] slack       🟦     │  Block Kit 인터랙티브
                └──────────────┬───────────┘
                               ▼
                          🟢 END
```
...
<details><summary>그래픽 다이어그램 펼치기 (19KB SVG)</summary>

<img src="docs/agent-workflow.svg" alt="Smart PR Inspector Workflow" width="700"/>
