...
SCENARIO_GENERATION_PROMPT = Template("""당신은 QA 전문가이자 도메인 분석가입니다.
아래 PR 변경사항을 보고, 코드를 모르는 도메인 전문가도 이해할 수 있는 테스트 시나리오를 작성하세요.
...
지침:
- 기술 용어 대신 비즈니스/도메인 언어로 작성
- "함수가 호출되면" 대신 "사용자가 X를 하면" 형태로 작성
- 실제로 중요한 케이스만 4~6개 선정 (기능, 예외, 경계값, 보안 포함)
- 각 시나리오는 독립적이고 명확하게

JSON 배열 형식으로만 출력하세요 (설명 없이):
[
  {
    "id": "s1",
    "title": "시나리오 한 줄 제목",
    "given": "전제 조건 (현재 상태나 사전 조건)",
    "when": "어떤 동작을 수행했을 때",
    "then": "어떤 결과가 나와야 하는가",
    "category": "기능|예외|경계값|보안"
  }
]""")
...
SCENARIO_EVALUATION_PROMPT = Template("""당신은 코드 리뷰 전문가입니다.
아래 코드 변경(diff)을 분석하여, 주어진 시나리오가 올바르게 구현되었는지 평가하세요.
...
평가 기준:
- diff에서 이 시나리오를 충족하는 코드가 실제로 추가/수정되었는가?
- 예외 처리나 엣지 케이스가 누락되지 않았는가?
- 코드 로직이 "Then"의 기대 결과를 보장하는가?

다음 JSON 형식으로만 응답하세요:
{
  "verdict": "pass|fail|unclear",
  "confidence": 0~100,
  "reasoning": "한국어로 판단 근거를 2~3문장으로 설명. 도메인 전문가가 이해할 수 있는 평문으로 작성."
}""")


# ── 비즈니스 영향도 분석 프롬프트 ─────────────────────────────────────────

BUSINESS_IMPACT_PROMPT = Template("""당신은 시니어 엔지니어이자 비즈니스 분석가입니다.
아래 코드 변경사항을 비즈니스 관점에서 분석하세요. 기술적 세부사항보다 비즈니스 임팩트에 집중하세요.

PR 제목: $pr_title
변경된 파일: $changed_files
변경된 함수: $changed_functions
영향받는 모듈: $affected_modules
API 변경 여부: $has_api_changes
코드 리스크 레벨: $risk_level

코드 변경 요약:
$diff_snippet

분석 관점:
1. 이 변경이 최종 사용자에게 어떤 영향을 미치는가?
2. 어떤 비즈니스 기능/피처가 영향을 받는가?
3. 장애 발생 시 비즈니스에 어떤 손실이 예상되는가?
4. 시니어 엔지니어로서 배포 전 추가로 확인해야 할 사항은?

다음 JSON 형식으로만 응답하세요 (설명 없이):
{
  "summary": "비즈니스 관점 한 줄 요약 (50자 이내)",
  "affected_features": ["영향받는 사용자 기능 목록"],
  "user_facing_changes": "최종 사용자가 체감할 변화 설명 (2-3문장)",
